...
export class AlbumController {
  ...
  @Get(':id')
  getAlbum(@Param('id') id: string, @Res({ passthrough: true }) res: Response): Album {
    if (!uuidValidate(id)) {
  ...
  @Post()
  createAlbum(@Body() createAlbumDto: CreateAlbumDto, @Res({ passthrough: true }) res: Response): Album {
  ...
  @Put(':id')
  updateAlbum(@Body() updateAlbumDto: UpdateAlbumDto, @Param('id') id: string, @Res({ passthrough: true }) res: Response): Album {
  ...
  @Delete(':id')
  deleteAlbum(@Param('id') id: string, @Res({ passthrough: true }) res: Response) {
    if (!uuidValidate(id)) {
      res.status(HttpStatus.BAD_REQUEST)
    } else {
