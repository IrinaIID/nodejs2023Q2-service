...
export class TrackController {
  ...
  @Post()
  createTrack(@Body() createTrack: CreateTrackDto, @Res({ passthrough: true }) res: Response) {
  ...
  @Put(':id')
  updateTrack(@Body() updateTrack: UpdateTrackDto, @Param('id') id: string, @Res({ passthrough: true }) res: Response) {
    if (!uuidValidate(id)) {
      res.status(HttpStatus.BAD_REQUEST)
    } else {
  ...
  @Delete(':id')
  deleteTrack(@Param('id') id: string, @Res({ passthrough: true }) res: Response) {
    if (!uuidValidate(id)) {
      res.status(HttpStatus.BAD_REQUEST)
    } else {
